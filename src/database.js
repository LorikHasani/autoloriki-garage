...
console.log(`🗄️  Database Mode: ${USE_SUPABASE ? 'Supabase (Cloud)' : 'localStorage (Local)'}`);
...
export const customersDB = {
  ...
  async create(customer) {
    if (!USE_SUPABASE) {
      ...
      localStorage.setItem('garazh_customers', JSON.stringify([...all, newCustomer]));
      return newCustomer;
    }
...
export const vehiclesDB = {
  ...
  async create(vehicle) {
    if (!USE_SUPABASE) {
      ...
      localStorage.setItem('garazh_vehicles', JSON.stringify([...all, newVehicle]));
      return newVehicle;
    }
...
export const dbUtils = {
  ...
  async testConnection() {
    if (!USE_SUPABASE) {
      return { success: true, mode: 'localStorage', message: 'Using local storage' };
    }
    ...
    try {
      ...
      return { success: true, mode: 'Supabase', message: 'Connected to Supabase' };
    } catch (error) {
...
export const vehiclesDB = {
  async getAll() {
    ...
    return data.map(v => ({
      id: uuidToId(v.id),
      customerId: uuidToId(v.customer_id),
      make: v.make,
      model: v.model,
      year: v.year || '',
      plate: v.plate,
  ...
  async create(vehicle) {
    ...
    const { data, error } = await supabase
      .from('vehicles')
      .insert([{
        customer_id: idToUUID(vehicle.customerId),
        make: vehicle.make,
        model: vehicle.model,
        year: vehicle.year || null,
        plate: vehicle.plate,
    ...
    return {
      id: uuidToId(data.id),
      customerId: uuidToId(data.customer_id),
      make: data.make,
      model: data.model,
      year: data.year || '',
      plate: data.plate,
...
export const ordersDB = {
  async getAll() {
    ...
    return data.map(o => ({
      id: uuidToId(o.id),
      vehicleId: uuidToId(o.vehicle_id),
      customerId: uuidToId(o.customer_id),
      status: o.status,
      startDate: o.start_date,
      endDate: o.end_date || null,
      paid: o.paid,
  ...
  async create(order) {
    ...
    const { data, error } = await supabase
      .from('service_orders')
      .insert([{
        vehicle_id: idToUUID(order.vehicleId),
        customer_id: idToUUID(order.customerId),
        status: order.status,
        start_date: order.startDate,
        end_date: order.endDate || null,
        paid: order.paid || false,
    ...
    return {
      id: uuidToId(data.id),
      vehicleId: uuidToId(data.vehicle_id),
      customerId: uuidToId(data.customer_id),
      status: data.status,
      startDate: data.start_date,
      endDate: data.end_date || null,
