...
function Customers({ customers, setCustomers, vehicles, orders, dailyLog }) {
  ...
  const deleteCustomer = async (id, name) => {
    if (!window.confirm(`Jeni i sigurt që dëshironi të fshini klientin ${name}?`)) return;
    
    try {
      const db = await import('./database.js');
      await db.customersDB.delete(id);
      ...
      <div className="table-wrap"><table><thead><tr><th>#</th><th>Name</th><th>Phone</th><th>Email</th><th>Vehicles</th><th>Address</th><th style={{width:90}}>Actions</th></tr></thead>
        <tbody>{filtered.map((c,i)=>{
          ...
          <tr key={c.id}><td style={{color:"#5a5f72",fontSize:12}}>{i+1}</td>
            ...
            <td>{c.address}</td>
            <td><div className="action-btns"><button className="icon-btn" onClick={()=>openEdit(c)}><Icon type="edit" size={14}/></button><button className="icon-btn danger" onClick={()=>deleteCustomer(c.id, c.name)}><Icon type="trash" size={14}/></button></div></td>
        </tr>);})}</tbody></table>
...
function Vehicles({ vehicles, setVehicles, customers }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      const vehicleData = { 
        ...form, 
        customerId: form.customerId, // Keep as string (UUID)
        year: Number(form.year) || null 
      };
      
      if (editing) {
        // Update existing vehicle
        const updated = await db.vehiclesDB.update(editing, vehicleData);
        setVehicles(prev => prev.map(v => v.id === editing ? updated : v));
      } else {
        // Create new vehicle
        const newVehicle = await db.vehiclesDB.create(vehicleData);
      ...
      alert('Gabim gjatë ruajtjes së automjetit!');
    }
  };
  
  const deleteVehicle = async (id, plate) => {
    if (!window.confirm(`Jeni i sigurt që dëshironi të fshini automjetin ${plate}?`)) return;
    
    try {
      const db = await import('./database.js');
      await db.vehiclesDB.delete(id);
      setVehicles(p => p.filter(v => v.id !== id));
    } catch (error) {
      console.error('❌ Delete vehicle error:', error);
      alert('Gabim gjatë fshirjes së automjetit!');
    }
  };
  return (
    <div>
      ...
      <div className="table-wrap"><table><thead><tr><th>#</th><th>Automjeti</th><th>Targa</th><th>VIN</th><th>Viti</th><th>Ngjyra</th><th>Pronari</th><th style={{width:90}}>Veprimet</th></tr></thead>
        <tbody>{filtered.map((v,i)=>{
          ...
          return <tr key={v.id}><td style={{color:"#5a5f72",fontSize:12}}>{i+1}</td><td style={{color:"#fff",fontWeight:500}}>{v.make} {v.model}</td>
            ...
            <td><div className="action-btns"><button className="icon-btn" onClick={()=>openEdit(v)}><Icon type="edit" size={14}/></button><button className="icon-btn danger" onClick={()=>deleteVehicle(v.id, v.plate)}><Icon type="trash" size={14}/></button></div></td>
          </tr>;
        })}</tbody></table>
...
function ServiceOrders({ orders, setOrders, dailyLog, setDailyLog, vehicles, customers, serviceTypes }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      const orderData = {
        vehicleId:  form.vehicleId,  // Keep as UUID string
        customerId: form.customerId, // Keep as UUID string
        status:     form.status,
        startDate:  form.startDate || TODAY,
        endDate:    form.status==="Përfunduar" ? (form.endDate || TODAY) : null,
        notes:      form.notes,
        paid:       !!form.paid,
      ...
      if (editing) {
        ...
        const updated = await db.ordersDB.update(editing, orderData);
        setOrders(p => p.map(o => o.id === editing ? updated : o));
      } else {
        // Create new order
        const newOrder = await db.ordersDB.create(orderData);
      ...
      alert('Gabim gjatë ruajtjes së porosisë!');
    }
  };
  ...
  const markComplete = async (o) => {
    try {
      const db = await import('./database.js');
      const updated = await db.ordersDB.update(o.id, { 
        ...o,
        status: "Përfunduar", 
        endDate: TODAY 
      });
  ...
  const togglePaid = async (id) => {
    try {
      ...
      const updated = await db.ordersDB.update(id, { ...order, paid: !order.paid });
  ...
  const deleteOrder = async (id) => {
    if (!window.confirm('Jeni i sigurt që dëshironi të fshini këtë porosi?')) return;
    
    try {
      const db = await import('./database.js');
      await db.ordersDB.delete(id);
          ...
          <tbody>{orders.map((o,i)=>{
            ...
            return <tr key={o.id}>
              ...
              <td><div className="action-btns">
                ...
                <button className="icon-btn danger" onClick={()=>deleteOrder(o.id)}><Icon type="trash" size={14}/></button>
              </div></td>
            </tr>;
          })}</tbody>
        </table>
...
function Invoices({ orders, setOrders, dailyLog, setDailyLog, vehicles, customers }) {
  ...
  const togglePaidInv = async (id) => {
    try {
      const db = await import('./database.js');
      
      // Find order in either orders or dailyLog
      const orderInActive = orders.find(o => o.id === id);
      const orderInArchive = dailyLog.find(o => o.id === id);
      const order = orderInActive || orderInArchive;
      ...
      // Update in correct state
      if (orderInActive) {
        setOrders(p => p.map(o => o.id === id ? updated : o));
      } else if (orderInArchive) {
        setDailyLog(p => p.map(o => o.id === id ? updated : o));
      }
    } catch (error) {
      console.error('❌ Toggle paid error:', error);
      alert('Gabim gjatë ndryshimit të statusit të pagesës!');
