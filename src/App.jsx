...
const INITIAL_ORDERS = [
  {
    ...
    endDate: null,
    paid: false,
    notes: "Frenat përpara + mbushje lëngu",
    services: [
      {
      ...
      },
      {
        ...
        laborPrice: 20,
        parts: [{ name: "Lëng Frenash", qty: 1, costPrice: 12, sellPrice: 22 }],
      },
    ],
  },
  {
    ...
    services: [
      {
        serviceType: "Ndërrimi i Gomave",
        laborPrice: 60,
        parts: [
          { name: "Goma 205/55 R16", qty: 4, costPrice: 70, sellPrice: 105 },
        ],
      },
    ],
  },
...
const INITIAL_DAILY_LOG = [
  ...
  },
  {
    ...
    endDate: "2026-01-20",
    paid: true,
    notes: "Servisim i madh + inspektim",
    services: [
      {
        serviceType: "Riparimi i Motorit",
        laborPrice: 300,
        parts: [
          { name: "Rrip Kohe", qty: 1, costPrice: 45, sellPrice: 90 },
          { name: "Pompë Uji", qty: 1, costPrice: 60, sellPrice: 110 },
          { name: "Set Guarnicionesh", qty: 1, costPrice: 35, sellPrice: 65 },
        ],
      },
      {
        serviceType: "Inspektimi i Përgjithshëm",
        laborPrice: 50,
        parts: [{ name: "Filtër Ajri", qty: 1, costPrice: 10, sellPrice: 22 }],
      },
    ],
  },
  {
    ...
    endDate: "2026-01-28",
    paid: false,
    notes: "Kontroll vjetor",
    services: [
      {
        serviceType: "Inspektimi i Përgjithshëm",
        laborPrice: 55,
        parts: [{ name: "Filtër Ajri", qty: 1, costPrice: 10, sellPrice: 20 }],
      },
    ],
  },
  {
    ...
    endDate: "2026-01-28",
    paid: true,
    notes: "Çarje e riparuar",
    services: [
      {
        serviceType: "Riparimi i Xhamit",
        laborPrice: 45,
        parts: [{ name: "Kit Riparimi", qty: 1, costPrice: 15, sellPrice: 30 }],
      },
    ],
  },
];
...
function Dashboard({ customers, vehicles, orders, dailyLog }) {
  ...
  const recentLog = [...filtered]
    .sort((a, b) => new Date(b.endDate) - new Date(a.endDate))
    .slice(0, 4)
    .map((o) => ({ ...o, ...calcOrder(o) }));
  const laborIncome = completedCalcs.reduce((s, o) => s + o.labor, 0);
  const partsMarkup = completedCalcs.reduce(
    (s, o) => s + (o.partsSell - o.partsCost),
    0,
  );
      ...
      <div className="dash-grid">
        <div className="dash-card amber">
          ...
          <div className="dash-sub">
            nga {filtered.length} porosi të përfunduara
          </div>
        </div>
        <div className="dash-card red">
          ...
          <h3>${totalCOGS.toLocaleString()}</h3>
          <p>Kostoja e Pjesëve (COGS)</p>
          <div className="dash-sub">shpenzuar për pjesë</div>
        </div>
        ...
        <div className="dash-card blue">
          ...
          <p>Aktive Sot</p>
          <div className="dash-sub">
            {customers.length} klientë · {vehicles.length} automjete
          </div>
        </div>
      </div>
      <div className="two-col">
        <div className="table-wrap">
          ...
          {recentLog.map((o) => {
            ...
            const svcNames = (o.services || [])
              .map((s) => s.serviceType)
              .join(", ");
            return (
              <div className="order-mini" key={o.id}>
                <div className="order-mini-icon">
                  <Icon type="wrench" size={17} />
                </div>
                <div style={{ flex: 1 }}>
                  <p
                    style={{ fontSize: 13, color: "#e2e4e9", fontWeight: 500 }}
                  >
                    {svcNames} — {v ? `${v.make} ${v.model}` : "—"}
                  </p>
                  <span style={{ fontSize: 11.5, color: "#5a5f72" }}>
            ...
            {[
              ...
              >
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  ...
                  />
                  <span style={{ fontSize: 13, color: "#c0c4d0" }}>
                    {item.label}
                  </span>
                </div>
                <span
  ...
  return (
    <div className="hist-panel">
      <button className="hist-back" onClick={onBack}>
        <Icon type="back" size={16} /> Back to Customers
      </button>
      ...
      <div className="hist-stats-row">
        ...
        <div className="hist-stat-card">
          <b>{custVehicles.length}</b>
          <span>Vehicles</span>
        </div>
        <div className="hist-stat-card revenue">
          <b>${totalRevenue.toLocaleString()}</b>
          <span>Total Spent</span>
        </div>
        <div className="hist-stat-card profit">
          <b>${totalProfit.toLocaleString()}</b>
            ...
            {allOrders.map((o) => {
              ...
              return [
                <tr key={o.id}>
                  ...
                  <td
                    style={{
                      color: calc.profit >= 0 ? "#10b981" : "#ef4444",
                      fontWeight: 700,
                    }}
                  >
                    ${calc.profit}
                    ...
                    <td colSpan={7} style={{ padding: 0 }}>
                      <div className="inv-detail">
                        {(o.services || []).map((svc, si) => (
                          <div key={si}>
                            <div className="inv-svc-label">
                              {svc.serviceType}
                            </div>
                            <div className="row">
                              <span className="lbl">&nbsp;&nbsp;Labor</span>
                              <span className="val">${svc.laborPrice}</span>
                            </div>
                            {(svc.parts || []).map((p, pi) => (
                              <div className="row" key={pi}>
                                ...
                                <span className="val">
                                  Sell ${p.sellPrice} · Cost ${p.costPrice} ·{" "}
                        ...
                        <div className="row">
                          ...
                          <span className="val green" style={{ fontSize: 14 }}>
                            ${calc.profit}
                          </span>
                        </div>
                      </div>
  ...
  const save = async () => {
    ...
    try {
      ...
      if (editing) {
        ...
        const updated = await db.customersDB.update(editing, form);
        setCustomers((p) => p.map((c) => (c.id === editing ? updated : c)));
      } else {
        // Create new customer
        const newCustomer = await db.customersDB.create(form);
    ...
    <div>
      ...
      >
        <div className="search-bar" style={{ width: 320 }}>
          ...
          <input
            placeholder="Search by name or plate (e.g. 06-554-GR)…"
            value={search}
            onChange={(e) => setSearch(e.target.value)}
          />
        </div>
        <button className="btn btn-primary" onClick={openAdd}>
          <Icon type="plus" size={15} /> Add Customer
        </button>
      </div>
      <div className="table-wrap">
        <table>
            ...
            {filtered.map((c, i) => {
              ...
              return (
                <tr key={c.id}>
                  ...
                  <td>{c.phone}</td>
                  <td style={{ color: "#6b8cce" }}>{c.email}</td>
                  <td>
                    {cVehicles.map((v) => (
                      <span className="plate-chip" key={v.id}>
                        {v.plate}
                      </span>
                    ))}
                  </td>
                    ...
                    <div className="action-btns">
                      ...
                      <button
                        ...
                        onClick={() => {
                          if (
                            window.confirm(
                              `Jeni i sigurt që dëshironi të fshini klientin ${c.name}?`,
                            )
                          ) {
...
function Vehicles({ vehicles, setVehicles, customers }) {
  ...
  const filtered = vehicles.filter((v) => {
    ...
    return `${v.make} ${v.model} ${v.plate} ${v.vin || ""} ${c?.name || ""}`
      .toLowerCase()
  ...
  const save = () => {
    ...
    editing
      ? setVehicles((prev) =>
          prev.map((v) => (v.id === editing ? { ...v, ...p } : v)),
        )
      : setVehicles((prev) => [...prev, { id: Date.now(), ...p }]);
    setModal(false);
  };
  return (
    <div>
      ...
      >
        <div className="search-bar" style={{ width: 280 }}>
          <Icon type="search" size={15} />
          <input
            placeholder="Kërko automjete (emër, targë, VIN)…"
            value={search}
            onChange={(e) => setSearch(e.target.value)}
          />
        </div>
        <button className="btn btn-primary" onClick={openAdd}>
          <Icon type="plus" size={15} /> Shto Automjet
        </button>
      </div>
      <div className="table-wrap">
        <table>
            ...
            {filtered.map((v, i) => {
              ...
              return (
                <tr key={v.id}>
                  ...
                  <td>{v.year}</td>
                  <td>{v.color}</td>
                  <td style={{ color: "#6b8cce" }}>{owner?.name || "—"}</td>
                  <td>
                    <div className="action-btns">
                      ...
                      <button
                        ...
                        onClick={() => {
                          if (
                            window.confirm(
                              `Jeni i sigurt që dëshironi të fshini automjetin ${v.plate}?`,
                            )
                          ) {
      ...
      {modal && (
        <div className="modal-overlay" onClick={() => setModal(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <div className="modal-head">
              <h3>{editing ? "Ndrysho" : "Regjistro"} Automjetin</h3>
              <button className="icon-btn" onClick={() => setModal(false)}>
                <Icon type="x" size={15} />
              </button>
            </div>
            <div className="modal-body">
              ...
              <div className="form-group">
                <label>VIN (Numri i Shasisë)</label>
                <input
                  className="inp"
                  value={form.vin}
...
function OrderModal({
  ...
  const handleCustomerChange = (val) => {
    const newCustVehicles = vehicles.filter(
      (v) => v.customerId === Number(val),
    );
    const vehicleStillValid = newCustVehicles.some(
      (v) => v.id === Number(form.vehicleId),
    );
    setForm((f) => ({
      ...f,
      customerId: val,
      vehicleId: vehicleStillValid ? f.vehicleId : "",
    }));
  };
    ...
    setForm((f) => ({
      ...
      services: [
        ...f.services,
        {
          ...
          laborPrice: "",
          parts: [{ name: "", qty: "1", costPrice: "", sellPrice: "" }],
        },
      ],
    }));
  const removeService = (si) =>
    setForm((f) => ({ ...f, services: f.services.filter((_, i) => i !== si) }));
  const updateService = (si, field, val) =>
    setForm((f) => ({
      ...f,
      ...
      services: f.services.map((s, i) =>
        i === si
          ? {
              ...
              parts: [
                ...s.parts,
                { name: "", qty: "1", costPrice: "", sellPrice: "" },
              ],
            }
          : s,
      ),
    }));
  const removePart = (si, pi) =>
    setForm((f) => ({
      ...f,
      services: f.services.map((s, i) =>
        i === si ? { ...s, parts: s.parts.filter((_, j) => j !== pi) } : s,
      ),
    }));
  const updatePart = (si, pi, field, val) =>
    setForm((f) => ({
      ...f,
      services: f.services.map((s, i) =>
        i === si
  ...
  const live = useMemo(() => {
    ...
    return {
      ...
      partsSell,
      labor,
      revenue: partsSell + labor,
      profit: partsSell + labor - partsCost,
    };
  }, [form.services]);
      ...
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-body">
          ...
          <div className="form-row">
            ...
            <div className="form-group">
              ...
              >
                <option value="">
                  {form.customerId ? "Select vehicle" : "Pick a customer first"}
                </option>
                {custVehicles.map((v) => (
          ...
          <div className="form-group">
            ...
            >
              {["Në Pritje", "Në Progres", "Përfunduar", "Anuluar"].map((s) => (
          ...
          <div style={{ marginBottom: 8 }}>
            ...
            >
              ...
              <button className="btn btn-ghost btn-sm" onClick={addService}>
                <Icon type="plus" size={13} /> Add Service
              </button>
            </div>
              ...
              <div className="svc-block" key={si}>
                ...
                <div className="svc-block-head">
                  <div className="svc-block-head-left">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        updateService(si, "serviceType", e.target.value)
                      }
                    >
                      {serviceTypes.map((s) => (
                        <option key={s} value={s}>
                ...
                <div className="svc-labor-row">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      updateService(si, "laborPrice", e.target.value)
                    }
                  />
                </div>
                ...
                <div className="parts-head">
                  ...
                  <button
                    ...
                    onClick={() => addPart(si)}
                  >
                    <Icon type="plus" size={11} /> Part
                  </button>
                </div>
                <div className="parts-col-labels">
                  <span>Part Name</span>
                  <span>Qty</span>
                  <span>Cost ($)</span>
                  <span>Sell ($)</span>
                ...
                {svc.parts.map((p, pi) => (
                  <div className="part-row" key={pi}>
                    <input
                      ...
                      value={p.name}
                      onChange={(e) =>
                        updatePart(si, pi, "name", e.target.value)
                      }
                    />
                    <input
                      className="inp"
                      ...
                      value={p.qty}
                      onChange={(e) =>
                        updatePart(si, pi, "qty", e.target.value)
                      }
                    />
                    <input
                      className="inp"
                      ...
                      value={p.costPrice}
                      onChange={(e) =>
                        updatePart(si, pi, "costPrice", e.target.value)
                      }
                    />
                    <input
                      className="inp"
                      ...
                      value={p.sellPrice}
                      onChange={(e) =>
                        updatePart(si, pi, "sellPrice", e.target.value)
                      }
                    />
                    <button
                      className="part-del"
                      onClick={() => removePart(si, pi)}
                    >
                      &times;
                    </button>
                  </div>
          ...
          <div className="summary-box">
            <div className="sum-row">
              <span className="lbl">Total Labor</span>
              <span className="val">${live.labor}</span>
            </div>
            <div className="sum-row">
              <span className="lbl">Parts Sold</span>
              <span className="val">${live.partsSell}</span>
            </div>
            <div className="sum-row">
              <span className="lbl" style={{ color: "#ef4444" }}>
                Parts Cost
              </span>
              <span className="val" style={{ color: "#ef4444" }}>
                ${live.partsCost}
            ...
            <div className="sum-row divider revenue">
              <span className="lbl">Revenue</span>
              <span className="val">${live.revenue}</span>
            </div>
            <div className="sum-row profit">
              <span className="lbl">Netto Profit</span>
              <span
                className="val"
                style={{ color: live.profit >= 0 ? "#10b981" : "#ef4444" }}
              >
                ${live.profit}
              </span>
            </div>
          ...
          <div className="toggle-wrap">
            ...
            <span className={`toggle-label ${form.paid ? "paid" : "unpaid"}`}>
        ...
        <div className="modal-footer">
          ...
          <button
            ...
            onClick={() => onPrint && onPrint(form)}
          >
            <Icon type="receipt" size={14} /> Print
          </button>
...
function ServiceOrders({
    ...
    setForm({
      ...
      services: o.services.map((s) => ({
        ...
        parts: s.parts.length
          ? s.parts.map((p) => ({ ...p, qty: p.qty != null ? p.qty : 1 }))
          : [{ name: "", qty: "1", costPrice: "", sellPrice: "" }],
      })),
    });
    setModal(true);
  ...
  const save = () => {
    if (!form.vehicleId || !form.customerId || form.services.length === 0)
      return;
    ...
    if (editing) {
      setOrders((p) =>
        p.map((o) => (o.id === editing ? { ...o, ...payload } : o)),
      );
    } else {
  ...
  const markComplete = (o) => {
    setOrders((p) =>
      p.map((x) =>
        x.id === o.id ? { ...x, status: "Përfunduar", endDate: TODAY } : x,
      ),
    );
  };
  ...
  const togglePaid = (id) =>
    setOrders((p) => p.map((o) => (o.id === id ? { ...o, paid: !o.paid } : o)));
  ...
  return (
    <div>
      <div className="today-banner">
        ...
        <p>
          Showing today's active orders — <span>{formatDate(TODAY)}</span>.
          Completed orders stay here until tomorrow, then move to Daily Log
          automatically.
        </p>
      </div>
      <div
      ...
      >
        <span style={{ fontSize: 13, color: "#6b7280" }}>
          {orders.length} active order{orders.length !== 1 ? "s" : ""} today
        </span>
        <button className="btn btn-primary" onClick={openAdd}>
          <Icon type="plus" size={15} /> New Order
        </button>
      </div>
      ...
      <div className="table-wrap">
        <table>
          ...
          <tbody>
            {orders.map((o, i) => {
              const v = vehicles.find((v) => v.id === o.vehicleId),
                c = customers.find((c) => c.id === o.customerId),
                calc = calcOrder(o);
              return (
                <tr key={o.id}>
                  <td style={{ color: "#5a5f72", fontSize: 12 }}>{i + 1}</td>
                  ...
                  <td>
                    <button
                      className={`pay-btn ${o.paid ? "paid" : "unpaid"}`}
                  ...
                  <td>
                    <div className="action-btns">
                      {o.status !== "Completed" && (
                        <button
                          className="icon-btn green-hover"
                      ...
                      <button
                        className="icon-btn print-hover"
                        onClick={() => setPrintFor(o)}
                        title="Print Invoice"
                      >
                        <Icon type="receipt" size={14} />
                      </button>
                      ...
                      <button
                        ...
                        onClick={() => {
                          if (
                            window.confirm(
                              "Jeni i sigurt që dëshironi të fshini këtë porosi?",
                            )
                          ) {
        ...
        {orders.length === 0 && (
          <p style={{ padding: "32px", textAlign: "center", color: "#5a5f72" }}>
            No active orders for today. Add a new one!
          </p>
        )}
      </div>
        ...
        <OrderModal
          ...
          onPrint={(f) => {
            ...
            const tmp = {
              ...
              status: f.status,
              startDate: TODAY,
              endDate: f.status === "Përfunduar" ? TODAY : null,
              paid: !!f.paid,
              notes: f.notes,
              services: f.services.map((svc) => ({
                serviceType: svc.serviceType,
                laborPrice: Number(svc.laborPrice) || 0,
                parts: svc.parts
                  .filter((p) => p.name)
    ...
    const createPrintContent = () => {
      ...
      (order.services || []).forEach((svc) => {
        ...
        (svc.parts || []).forEach((p) => {
          ...
          const partPrice = showPrices
            ? "$" + (Number(p.sellPrice) || 0) * qty
            : "—";
    ...
    <div className="print-overlay" onClick={onClose}>
      <div className="print-card" onClick={(e) => e.stopPropagation()}>
        ...
        <div style={{ padding: "32px", maxHeight: "60vh", overflowY: "auto" }}>
          ...
          >
            <h2 style={{ fontSize: 22, marginBottom: 4, color: "#0f1117" }}>
              🚗 AUTO BASHKIMI-L
            </h2>
          ...
          <div style={{ fontSize: 12, color: "#6b7280", marginBottom: 18 }}>
            Faturë · {formatDate(order.startDate)}
          </div>

          <div className="print-row">
            <span className="pr-lbl">Klienti</span>
            <span className="pr-val">{c?.name || "—"}</span>
          </div>
          <div className="print-row">
            <span className="pr-lbl">Telefoni</span>
            <span className="pr-val">{c?.phone || "—"}</span>
          </div>
          <div className="print-row">
            <span className="pr-lbl">Automjeti</span>
          ...
          {v?.vin && (
            <div className="print-row">
              <span className="pr-lbl">VIN</span>
              <span
                className="pr-val"
          ...
          {order.notes && (
            <div className="print-row">
              <span className="pr-lbl">Shënime</span>
              <span className="pr-val">{order.notes}</span>
            </div>
          )}
          ...
          {showPrices ? (
            <div className="print-totals">
              <div className="print-row">
                <span className="pr-lbl">Totali i Punës</span>
                <span className="pr-val">${calc.labor}</span>
              </div>
              <div className="print-row">
                <span className="pr-lbl">Totali i Pjesëve</span>
                <span className="pr-val">${calc.partsSell}</span>
              </div>
              <div
                className="print-row"
                style={{
              ...
              >
                ...
                <span className="pr-val" style={{ fontSize: 18 }}>
                  ${calc.revenue}
                </span>
              </div>
            </div>
            ...
            <div className="print-totals">
              ...
              >
                <span className="pr-lbl" style={{ fontWeight: 700 }}>
                  Shërbimet u Përfunduan
                </span>
                <span
                  className="pr-val"
    ...
    <div>
      <div style={{ marginBottom: 20 }}>
        <h3 style={{ fontSize: 16, color: "#fff", marginBottom: 16 }}>
          Menaxhimi i Llojeve të Shërbimit
        </h3>
      ...
      <div className="table-wrap">
        <div className="table-header">
          <h3>Llojet e Shërbimeve ({serviceTypes.length})</h3>
        </div>
        <table>
          <thead>
            <tr>
...
function DailyLog({ dailyLog, vehicles, customers }) {
  ...
  const onManualChange = (field, val) => {
    setPreset("manual");
  ...
  const grouped = useMemo(() => {
    const map = {};
    filtered.forEach((o) => {
      const key = o.endDate || o.startDate;
      if (!map[key]) map[key] = [];
      map[key].push(o);
    });
  ...
  return (
    <div>
      <div className="filter-bar">
        ...
        <div className="date-input-wrap">
          ...
          <input
            ...
            type="date"
            value={dateFrom}
            onChange={(e) => onManualChange("from", e.target.value)}
          />
        </div>
        <span className="sep">→</span>
        <div className="date-input-wrap">
          ...
          <input
            ...
            type="date"
            value={dateTo}
            onChange={(e) => onManualChange("to", e.target.value)}
          />
        </div>
      </div>
      <div className="finance-row" style={{ marginBottom: 20 }}>
        <div className="finance-card fc-blue">
          <p>Days Logged</p>
          <h4>{grouped.length}</h4>
          <div className="sub">unique days</div>
        </div>
        <div className="finance-card fc-amber">
          <p>Total Orders</p>
          <h4>{filtered.length}</h4>
          <div className="sub">completed services</div>
        </div>
        <div className="finance-card fc-amber">
          <p>Revenue</p>
          <h4>
        ...
        <div className="finance-card fc-green">
          <p>Netto Profit</p>
          <h4>
            $
            {filtered
          ...
          <div className="sub">after parts cost</div>
        </div>
      </div>
          ...
          <div className="day-block" key={date}>
            <div className="day-header">
              ...
              <div className="day-stats">
                <div className="day-stat count">
                  <span>Orders</span>
                  <b>{dayOrders.length}</b>
                </div>
                <div className="day-stat revenue">
                  <span>Revenue</span>
                  <b>${dayCalcs.reduce((s, c) => s + c.revenue, 0)}</b>
                </div>
                <div className="day-stat profit">
                  <span>Profit</span>
                  <b>${dayCalcs.reduce((s, c) => s + c.profit, 0)}</b>
                </div>
              </div>
            </div>
              ...
              <table>
                ...
                <tbody>
                  {dayOrders.map((o, i) => {
                    ...
                    return (
                      <tr key={o.id}>
                        <td style={{ color: "#5a5f72", fontSize: 12 }}>
                          {i + 1}
                        </td>
                        <td style={{ color: "#fff", fontWeight: 500 }}>
                          {c?.name || "—"}
      ...
      {grouped.length === 0 && (
        ...
        >
          <Icon type="archive" size={40} />
          <p style={{ marginTop: 12, fontSize: 14 }}>
            No completed services in this date range.
          </p>
        </div>
      )}
    </div>
...
function Invoices({
  ...
  const togglePaidInv = (id) => {
    if (orders.some((o) => o.id === id))
      setOrders((p) =>
        p.map((o) => (o.id === id ? { ...o, paid: !o.paid } : o)),
      );
    else if (dailyLog.some((o) => o.id === id))
      setDailyLog((p) =>
        p.map((o) => (o.id === id ? { ...o, paid: !o.paid } : o)),
      );
  };
    ...
    return [...dailyLog, ...orders]
      .filter((o) => {
        ...
        if (q) {
          ...
          const matchesPlate = v?.plate
            .toLowerCase()
            .replace(/[\s\-]/g, "")
            .includes(q);
          const matchesVIN = v?.vin
            ?.toLowerCase()
            .replace(/[\s\-]/g, "")
            .includes(q);
  ...
  const totalRevenue = completedFiltered
    .map((o) => calcOrder(o))
    .reduce((s, c) => s + c.revenue, 0);
  const totalCOGS = completedFiltered
    .map((o) => calcOrder(o))
    .reduce((s, c) => s + c.cogs, 0);
  const totalProfit = completedFiltered
    .map((o) => calcOrder(o))
    .reduce((s, c) => s + c.profit, 0);
  ...
  const unpaidRevenue = [...dailyLog, ...orders]
    ...
    <div>
      ...
      <div style={{ marginBottom: 18 }}>
        <div className="search-bar" style={{ width: 350 }}>
          <Icon type="search" size={15} />
          <input
            placeholder="Kërko sipas emrit, targës, ose VIN (p.sh. 1HGBH41JXMN)…"
            value={search}
            onChange={(e) => setSearch(e.target.value)}
          />
        </div>
      ...
      <div className="finance-row">
        <div className="finance-card fc-amber">
          ...
          <div className="sub">completed orders</div>
        </div>
        <div className="finance-card fc-red">
          <p>Total COGS</p>
          <h4>${totalCOGS.toLocaleString()}</h4>
          <div className="sub">parts purchased</div>
        </div>
        <div className="finance-card fc-green">
          <p>Netto Profit</p>
        ...
        <div className="finance-card fc-blue">
          <p>Pending Revenue</p>
          <h4>${pending.toLocaleString()}</h4>
          <div className="sub">open orders</div>
        </div>
        <div className="finance-card fc-orange">
          <p>Unpaid</p>
          <h4>${unpaidRevenue.toLocaleString()}</h4>
      ...
      <div className="pay-filter">
        <button
          className={`pay-pill ${payFilter === "all" ? "active-all" : ""}`}
          onClick={() => setPayFilter("all")}
        >
          All Invoices
        </button>
        <button
          className={`pay-pill ${payFilter === "paid" ? "active-paid" : ""}`}
          onClick={() => setPayFilter("paid")}
        >
          ✓ Paid
        </button>
        <button
          className={`pay-pill ${payFilter === "unpaid" ? "active-unpaid" : ""}`}
                            ...
                            {(svc.parts || []).map((p, pi) => {
                              ...
                              return (
                                <div className="row" key={pi}>
                                  ...
                                  <span className="val">
                                    Sold ${(Number(p.sellPrice) || 0) * qty} ·
                                    Cost ${(Number(p.costPrice) || 0) * qty} ·{" "}
                                    <span className="green">
                                      +$
...
export default function App() {
    ...
    const loadAllData = async () => {
      ...
      try {
        ...
        const active = ordersData.filter(
          (o) => o.status !== "Përfunduar" || !o.endDate || o.endDate >= today,
        );
        const archived = ordersData.filter(
          (o) => o.status === "Përfunduar" && o.endDate && o.endDate < today,
        );
  ...
  const titles = {
    ...
    customers: "Klientët",
    vehicles: "Automjetet",
    orders: "Porositë e Servisit",
    dailylog: "Regjistri Ditor",
    invoices: "Faturat",
    settings: "Cilësimet",
  };
        ...
        >
          ...
          >
            <div style={{ textAlign: "center", marginBottom: 24 }}>
              <h2 style={{ color: "#fff", marginBottom: 4, fontSize: 24 }}>
                🚗 AUTO BASHKIMI-L
              </h2>
              <p style={{ color: "#6b7280", fontSize: 13 }}>
                Menaxhimi i Garazhit
              </p>
            </div>
        ...
        <div className="app-root">
          <nav className="sidebar">
            <div className="sidebar-logo">
              <h1>
                🚗 AUTO<span>BASHKIMI-L</span>
              </h1>
              <p>Menaxhimi i Garazhit</p>
            </div>
            <div className="nav-items">
              ...
              >
                <button
                  ...
                  onClick={() => {
                    if (
                      window.confirm("Jeni i sigurt që dëshironi të dilni?")
                    ) {
                      setIsLoggedIn(false);
                      setPage("dashboard");
                    }
